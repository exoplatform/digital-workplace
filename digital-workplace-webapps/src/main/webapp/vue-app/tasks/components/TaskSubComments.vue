<template>
  <v-layout>
    <v-list-item-avatar size="30" class="mt-0">
      <v-img :src="getUserAvatar(subComment.author.username)"/>
    </v-list-item-avatar>
    <v-list-item-content
      class="pt-1 pb-0"
        ...
        <v-layout>
          <v-flex xs5>
            <v-list-item-title
              ...
              v-html="subComment.author.displayName"/>
          </v-flex>
          <v-flex xs6>
      ...
      <span class="taskContentComment" v-html="subComment.comment"></span>
      <v-btn
        id="reply_btn"
        depressed
        text
...
<script>
    export default {
        ...
        computed: {
            relativeTime() {
                return this.getRelativeTime(this.subComment.createdTime.time)
            }
        },
        methods: {
